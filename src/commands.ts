...
export function registerChatGroupCommands(ctx: Context, service: ChatGroupService): Disposer[] {
  ...
  disposers.push(ctx.commands.register(command('group', '群聊管理：create / add / remove / order / say / at / mention / status / stop / dissolve', 'create|add|remove|order|say|at|mention|status|stop|dissolve', async (invocation) => {
    try {
